    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-grow overflow-y-auto pr-2 space-y-3">
        <div className="grid grid-cols-12 gap-4 px-4 text-text-secondary font-semibold uppercase text-sm sticky top-0 bg-panel py-2">
          ...
          <span className="col-span-2 text-center">Almacén</span>
          <span className="col-span-2 text-right">Stock</span>
          <span className="col-span-1 text-right">Precio</span>
          <span className="col-span-2 text-right">Acciones</span>
        </div>
        
        {products.length > 0 && filteredProducts.length === 0 && <p className="text-text-secondary text-center pt-10">No se encontraron productos.</p>}
        ...
        {filteredProducts.map((product) => {
          ...
          return (
            ...
            >
              ...
              <span className="col-span-3 font-semibold text-text-primary truncate">{product.name}</span>
              <span className="col-span-2 text-text-secondary text-center font-bold">{product.almacen}</span>
